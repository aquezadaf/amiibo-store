...
it('does not call retrieve products on mount when there is at least one product', () => {
    const retrieveMock = jest.fn()
    const products = [{ id: 1 }]
    shallow(<ProductsScreen {...defaultProps} retrieveProducts={retrieveMock} products={products} />)
...
it('calls retrieve products when pressing retry', () => {
    const retrieveMock = jest.fn()
    const wrapper = shallow(<ProductsScreen {...defaultProps} products={[{}]} retrieveProducts={retrieveMock} error="Error" />)
    const retry = wrapper.find(Retry)
...
it('renders grid with products', () => {
    ...
    const wrapper = shallow(<ProductsScreen {...defaultProps} products={expectedProducts} />)
